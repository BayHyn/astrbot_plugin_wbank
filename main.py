...
class WbankPlugin(Star):
    ...
    async def delete_entry(self, event: AstrMessageEvent):
        ...
        if len(args) != 2 or not args[1].isdigit():
            yield event.plain_result("格式错误，应为：删除词条 关键词 序号")
            return
        keyword, idx = args[0], int(args[1])
        success = self.db.remove_entry_by_index(keyword, idx)
        if success:
            yield event.plain_result(f"已删除关键词【{keyword}】的第{idx}条词条")
        else:
            yield event.plain_result("删除失败，关键词或索引无效")
    ...
    async def set_alias(self, event: AstrMessageEvent):
        ...
        if len(args) < 2:
            yield event.plain_result("格式错误，应为：设置别名 关键词 别名1 别名2")
            return
        ...
        yield event.plain_result(f"已设置关键词【{keyword}】的别名：{aliases}")
    ...
    async def enable_keyword(self, event: AstrMessageEvent):
        ...
        if len(args) < 2:
            yield event.plain_result("格式错误，应为：启用别名 关键词 群1 群2")
            return
    ...
    async def disable_keyword(self, event: AstrMessageEvent, keyword: str):
        ...
        if len(args) < 2:
            yield event.plain_result("格式错误，应为：禁用关键词 关键词 群1 群2")
            return
        keyword, groups_id = args[0], args[1:]
        if keyword not in self.db.get_all_keywords():
            yield event.plain_result(f"关键词【{keyword}】不存在")
    ...
    async def list_all_keywords(self, event: AstrMessageEvent):
        ...
        if not keywords:
            ...
            return
        yield event.plain_result("📚 所有关键词：\n" + "、".join(keywords))
